<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-4">
      <h1 class="text-h4">Item Types</h1>
    ...
    <v-dialog v-model="showDialog" max-width="500">
      <v-card>
        <v-card-title>{{ editing ? 'Edit' : 'New' }} Item Type</v-card-title>
        <v-card-text>
          <v-text-field v-model="form.itemTypeName" label="Name" required />
...
import { GET_ITEM_TYPES, CREATE_ITEM_TYPE, UPDATE_ITEM_TYPE, DELETE_ITEM_TYPE } from './graphql';
const showSnackbar = inject<(t: string, c?: string) => void>('showSnackbar')!;
const headers = [{ title: 'ID', key: 'id' }, { title: 'Name', key: 'itemTypeName' }, { title: 'Description', key: 'description' }, { title: 'Actions', key: 'actions', sortable: false, width: 120 }];
const { result, loading, error, refetch } = useQuery(GET_ITEM_TYPES);
const items = ref<any[]>([]);
watch(() => result.value, (v) => { if (v) items.value = v.itemTypes; }, { immediate: true });
...
const form = ref({ itemTypeName: '', description: '' });
const { mutate: createM } = useMutation(CREATE_ITEM_TYPE);
const { mutate: updateM } = useMutation(UPDATE_ITEM_TYPE);
const { mutate: deleteM } = useMutation(DELETE_ITEM_TYPE);
function editItem(i: any) { editing.value = true; editingId.value = i.id; form.value = { itemTypeName: i.itemTypeName, description: i.description }; showDialog.value = true; }
function closeDialog() { showDialog.value = false; editing.value = false; form.value = { itemTypeName: '', description: '' }; }
