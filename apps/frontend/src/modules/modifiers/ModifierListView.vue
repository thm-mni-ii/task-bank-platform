<template>
  <div>
    ...
    <v-dialog v-model="showDialog" max-width="500">
      <v-card>
        <v-card-title>{{ editing ? 'Edit' : 'New' }} Modifier</v-card-title>
        <v-card-text>
          <v-text-field v-model="form.modifier" label="Modifier" required />
...
import { GET_MODIFIERS, CREATE_MODIFIER, UPDATE_MODIFIER, DELETE_MODIFIER } from './graphql';
const showSnackbar = inject<(t: string, c?: string) => void>('showSnackbar')!;
const headers = [{ title: 'ID', key: 'id' }, { title: 'Modifier', key: 'modifier' }, { title: 'Description', key: 'description' }, { title: 'Actions', key: 'actions', sortable: false, width: 120 }];
const { result, loading, error, refetch } = useQuery(GET_MODIFIERS);
const items = ref<any[]>([]);
watch(() => result.value, (v) => { if (v) items.value = v.modifiers; }, { immediate: true });
...
const form = ref({ modifier: '', description: '' });
const { mutate: createM } = useMutation(CREATE_MODIFIER);
const { mutate: updateM } = useMutation(UPDATE_MODIFIER);
const { mutate: deleteM } = useMutation(DELETE_MODIFIER);
function editItem(i: any) { editing.value = true; editingId.value = i.id; form.value = { modifier: i.modifier, description: i.description }; showDialog.value = true; }
function closeDialog() { showDialog.value = false; editing.value = false; form.value = { modifier: '', description: '' }; }
