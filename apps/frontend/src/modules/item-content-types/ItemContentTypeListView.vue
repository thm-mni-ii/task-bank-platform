<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-4">
      <h1 class="text-h4">Item Content Types</h1>
    ...
    <v-dialog v-model="showDialog" max-width="500">
      <v-card>
        <v-card-title>{{ editing ? 'Edit' : 'New' }} Content Type</v-card-title>
        <v-card-text>
          <v-text-field v-model="form.itemContentTypeName" label="Name" required />
...
import { GET_ITEM_CONTENT_TYPES, CREATE_ITEM_CONTENT_TYPE, UPDATE_ITEM_CONTENT_TYPE, DELETE_ITEM_CONTENT_TYPE } from './graphql';
const showSnackbar = inject<(t: string, c?: string) => void>('showSnackbar')!;
const headers = [{ title: 'ID', key: 'id' }, { title: 'Name', key: 'itemContentTypeName' }, { title: 'Description', key: 'description' }, { title: 'Actions', key: 'actions', sortable: false, width: 120 }];
const { result, loading, error, refetch } = useQuery(GET_ITEM_CONTENT_TYPES);
const items = ref<any[]>([]);
watch(() => result.value, (v) => { if (v) items.value = v.itemContentTypes; }, { immediate: true });
...
const form = ref({ itemContentTypeName: '', description: '' });
const { mutate: createM } = useMutation(CREATE_ITEM_CONTENT_TYPE);
const { mutate: updateM } = useMutation(UPDATE_ITEM_CONTENT_TYPE);
const { mutate: deleteM } = useMutation(DELETE_ITEM_CONTENT_TYPE);
function editItem(i: any) { editing.value = true; editingId.value = i.id; form.value = { itemContentTypeName: i.itemContentTypeName, description: i.description }; showDialog.value = true; }
function closeDialog() { showDialog.value = false; editing.value = false; form.value = { itemContentTypeName: '', description: '' }; }
